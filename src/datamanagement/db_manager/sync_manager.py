...
class SyncMixin:
    ...
    def sync_marker_ids(self):
        ...
        Uses COALESCE to treat NULL stop_dt as the far future.
    ...
    def sync_invalidation_ids(self):
        ...
        A NULL invalidation device_id is treated as applying to all devices.
        """
        with self.get_connection() as conn:
            conn.execute(
                """
                ...
                SET invalidation_id = (
                    ...
                    WHERE (ai.device_id IS NULL OR ai.device_id = ar.device_id)
