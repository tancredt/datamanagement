...
def filter_data(df, start_dt, stop_dt, interval, selected_sites, selected_gases,
                selected_devices, only_valid, group_by, data_type="area"):
    """
    ...
    if 'LOG TIME' in res.columns:
        ...
        # Convert to pd.Timestamp to avoid tz-naive vs tz-aware comparison errors
    ...
    # Normalize SITE and DEVICE to handle missing/empty values (leave as empty string)
    if 'SITE' in res.columns:
        res['SITE'] = res['SITE'].fillna('').astype(str).str.strip()
    if 'DEVICE' in res.columns:
        res['DEVICE'] = res['DEVICE'].fillna('').astype(str).str.strip()

    # Filter by selected sites and devices
    ...
    # ──────────────────────────────────────────────
    # SPOT & SPECTRAL SHORT-CIRCUIT
    # ──────────────────────────────────────────────
    # Spot and Spectral data are point-in-time readings. They do not use 
    # the continuous 1-minute bin coverage, interval aggregation, or INVALID flags.
    if data_type in ["spot", "spectral"]:
        return res

    # ──────────────────────────────────────────────
    # 2. COLUMN SELECTION (Area Data Only)
    # ──────────────────────────────────────────────
    ...
    # Only filter by the dimension that is actually being grouped (Area Data Logic)
    if group_by == 'Site':
        if 'DEVICE' in res.columns and selected_devices:
            res = res[res['DEVICE'].isin(selected_devices)]
    ...
    if only_valid and gas_cols_to_keep:
        for gas in gas_cols_to_keep:
            inv_col = f"INVALID_{gas}"
            if inv_col in res.columns:
                is_valid = pd.to_numeric(res[inv_col], errors='coerce').fillna(0) == 0
                res = res[is_valid]
        if res.empty:
            return res
    ...
    if interval_mins > 0:
        ...
        # Merge invalid flags back
        if invalid_cols_to_keep:
            flag_df = res.set_index('LOG TIME').copy()
            for inv_col in invalid_cols_to_keep:
                flag_df[inv_col] = pd.to_numeric(flag_df[inv_col], errors='coerce').fillna(0)
            flag_df = flag_df.groupby(
                [group_col, pd.Grouper(freq=f'{interval_mins}min', closed='right', label='right')]
            ).max().reset_index()
    ...
    # ──────────────────────────────────────────────
    if gas_cols_to_keep and interval_mins > 0:
