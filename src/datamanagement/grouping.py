...
def aggregate_data(df, interval, group_by, data_type="area", only_valid=False):
    """
    ...
    # STEP 2: Exclude invalid values from mean/min/max/count if only_valid is True.
    #
    ...
    # If "only_valid" is True, remove the aggregated values for bins that 
    # failed the 1-minute coverage rule.
    if only_valid:
        for analyte in analyte_cols:
            inv_col = f"INVALID_{analyte}"
            invalid_mask = res_agg[inv_col] == 1
            if analyte in res_agg.columns:
                res_agg.loc[invalid_mask, analyte] = np.nan
            for suffix in ["_min", "_max", "_count"]:
                col = f"{analyte}{suffix}"
                if col in res_agg.columns:
                    res_agg.loc[invalid_mask, col] = np.nan
...
def calculate_summary_dataframe(df, group_col, valid_analytes, is_exposure=False, only_valid=False):
    """
    ...
    Args:
        ...
        only_valid: If True, exclude rows where INVALID_{analyte} > 0.
    ...
    for group_val in groups:
        ...
        for analyte in valid_analytes:
            inv_col = f"INVALID_{analyte}"
            if only_valid and inv_col in group_df.columns:
