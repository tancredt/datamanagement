...
def update_site_from_device_locations(incident_path):
    processed_file = os.path.join(incident_path, "data", "processed", "area_data.csv")
    locations_file = os.path.join(incident_path, "mapping", "locations.json") # Changed path
    ...
    try:
        ...
        for loc in data.get("maps", {}).get("locations", []):
            for marker in loc.get("markers", []):
                device_locations.extend(marker.get("device_locations", []))
                
        for log_entry in device_locations:
...
def update_validations(incident_path):
    """
    Reads device_validations.json and updates the INVALID_{analyte} columns 
    in area_data.csv based on validation start/stop times.
    """
    processed_file = os.path.join(incident_path, "data", "processed", "area_data.csv")
    validations_file = os.path.join(incident_path, "validations", "device_validations.json")
    ...
    try:
        ...
        for val_entry in data.get("devices", []):
            ...
            analytes = val_entry.get("analytes", [])
            
            if not device_label or not start_time or not analytes:
                continue
