...
class DataView(QWidget):
    ...
    def _load_all_with_progress(self):
        ...
        try:
            # Step 1: Load configs (analytes from DB, thresholds from JSON)
            progress.setLabelText("Loading configuration...")
            QApplication.processEvents()
            self._load_configs()
            
            # Step 2: Load filters FIRST (needed before querying DB)
            progress.setLabelText("Loading filters...")
            QApplication.processEvents()
            self._load_filter_summary()
            
            # Step 3: Load raw data FROM DB with filters applied
    ...
    def set_filter_summary(self, filter_summary):
        ...
        # Re-query the database with the new filters
        self._load_raw_data()
        self._apply_aggregation()
    ...
    def _load_configs(self):
        ...
        # Load thresholds from JSON (still file-based)
    ...
    def _get_default_filters(self):
        ...
        # ✅ Use db_manager to get time range
        if self.db:
            try:
        ...
        # Get available devices and sites from DB
    ...
    # Kept for backward compatibility with any code calling apply_filters()
