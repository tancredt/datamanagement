...
class DataView(QWidget):
    ...
    def _load_all(self):
        """Load configs, raw data, filters, and apply them."""
        self._load_configs()
        self._load_raw_data()
    ...
    def refresh(self):
        """Reload filter summary from disk and re-apply filters."""
        self._load_filter_summary()
        self.apply_filters()
        self._render()
    
    def set_filter_summary(self, filter_summary):
        """Override the loaded filter summary (used by report_generator)."""
    ...
    def _load_filter_summary(self):
        """Load filter summary from meta/last_filters_{data_type}.json."""
        if not self.incident_path or not self.data_type:
            self.filter_summary = {}
            return
        
        filters_file = os.path.join(
            self.incident_path, "meta", f"last_filters_{self.data_type}.json"
        )
    ...
    def apply_filters(self):
        ...
        if self.filtered_data is not None and not self.filtered_data.empty:
            ...
            if interval != "Raw" and self.data_type != "exposure":
