...
class GPSAnalysisDialog(QDialog):
    ...
    def init_ui(self):
        ...
        layout.addWidget(
            QLabel("<b>Stable Locations</b> (> 30 mins, ≤ 20m consecutive drift):")
        )
    ...
    def _load_and_process_data(self, target_device):
        if not self.db:
            QMessageBox.critical(
                self,
                "Error",
                "No incident database is available.",
            )
            return
        ...
        try:
            with self.db.get_connection() as conn:
                rows = conn.execute(query, (target_device,)).fetchall()
        except Exception as e:
            QMessageBox.critical(
                self,
                "Error",
                f"Failed to read area data from database:\n{e}",
            )
            return
        ...
        if not rows:
            QMessageBox.warning(
                self,
                "No Data",
                f"No GPS data found for device: {target_device}",
            )
            self._clear_ui()
            return
        ...
        if df.empty:
            QMessageBox.warning(
                self,
                "No Data",
                "No valid GPS data remaining after cleaning.",
            )
            self._clear_ui()
            return
        ...
        if df_clean.empty:
            QMessageBox.warning(
                self,
                "No Data",
                "No valid data remaining after outlier filtering.",
            )
            self._clear_ui()
            return
        ...
        stable_groups["duration"] = (
            stable_groups["end_time"] - stable_groups["start_time"]
        )

        stable_locations = stable_groups[
            stable_groups["duration"] > pd.Timedelta(minutes=30)
        ].copy()

        stable_locations["duration_minutes"] = (
            stable_locations["duration"].dt.total_seconds() / 60
        )
    ...
    def update_plot(self, df_clean, target_device):
        ...
        self.ax1.set_title(
            f"Latitude & Longitude vs Log Time for {target_device}"
        )
    ...
    def update_table(self, result_table):
        ...
        for row_idx, (_, row) in enumerate(result_table.iterrows()):
            self.table.setItem(
                row_idx,
                0,
                QTableWidgetItem(str(row["Device"])),
            )
            self.table.setItem(
                row_idx,
                1,
                QTableWidgetItem(f"{row['Latitude']:.6f}"),
            )
            self.table.setItem(
                row_idx,
                2,
                QTableWidgetItem(f"{row['Longitude']:.6f}"),
            )
            self.table.setItem(
                row_idx,
                3,
                QTableWidgetItem(str(row["Start Time"])),
            )
            self.table.setItem(
                row_idx,
                4,
                QTableWidgetItem(str(row["End Time"])),
            )
            self.table.setItem(
                row_idx,
                5,
                QTableWidgetItem(f"{row['Duration (Minutes)']:.1f}"),
            )
            self.table.setItem(
                row_idx,
                6,
                QTableWidgetItem(str(row["Data Points"])),
            )
