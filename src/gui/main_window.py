...
class DataAnalyzerGUI(QMainWindow):
    ...
    def _update_app_state(self, has_incident: bool):
        actions = [
            self.action_edit_incident, self.action_maps, self.action_spot_readings,
    ...
    def _read_filter_file(self):
        """Reads the filter file for the current data type from disk."""
        if not self.active_incident_path or not self.data_type:
            return {}
        
        filters_file = os.path.join(
            self.active_incident_path, "meta", f"last_filters_{self.data_type}.json"
        )
    ...
    def _load_view(self, index):
        ...
        if index == 0:
            ...
            # Handle plume animation if data type is plume
            if self.data_type == "plume":
                ...
                view.show_plume_animation(plume_data)
