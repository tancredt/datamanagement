...
class ObservationWidget(QWidget):
    ...
    def __init__(self, data_type="spot", parent=None):
        ...
        if data_type == "area": self.data_type_combo.setCurrentText("Area Readings")
        elif data_type == "spectral": self.data_type_combo.setCurrentText("Spectral Results")
        elif data_type == "exposure": self.data_type_combo.setCurrentText("Exposures")
        elif data_type == "plume": self.data_type_combo.setCurrentText("Plumes")
        else: self.data_type_combo.setCurrentText("Spot Readings")
    ...
    def _on_data_type_changed(self, text):
        if "Spot" in text: self.data_type = "spot"
        elif "Area" in text: self.data_type = "area"
        elif "Exposures" in text: self.data_type = "exposure"
        elif "Plumes" in text: self.data_type = "plume"
        else: self.data_type = "spectral"
        self._update_form_for_data_type(self.data_type)
...
class ObjectiveWidget(QWidget):
    ...
    def __init__(self, objective_number=1, data_type="spot", parent=None):
        ...
        # Objective & Strategy Group
        objective_group = QGroupBox("Objective & Strategy")
        ...
        # ✅ NEW: Strategy text box
        self.strategy_text = QTextEdit()
        self.strategy_text.setMaximumHeight(80)
        self.strategy_text.setPlaceholderText("Enter the strategy to achieve the objective...")
        objective_layout.addWidget(self.strategy_text)
    ...
    def add_observation(self, form_text=None, filter_data=None, data_type=None):
        ...
        if form_text and form_text in [obs_widget.form_combo.itemText(i) for i in range(obs_widget.form_combo.count())]:
    ...
    def get_data(self):
        ...
        return {
            ...
            'strategy': self.strategy_text.toPlainText(), # ✅ NEW
...
class ObjectiveDialog(QDialog):
    ...
    def __init__(self, parent=None, incident_path=None, zone_name="", data_type="spot"):
        ...
        self.objectives_file = os.path.join(self.reports_dir, "objectives.json") if self.reports_dir else None
    ...
    def add_objective(self, tab_title=None, obj_data=None):
        if isinstance(tab_title, bool): tab_title = None
        ...
        if obj_data:
            ...
            if obj_widget.created: obj_widget.lbl_created.setText(f"Created: {obj_widget.created}")
            if obj_widget.updated: obj_widget.lbl_updated.setText(f"Updated: {obj_widget.updated}")
            
            obj_widget.objective_text.setPlainText(obj_data.get('objective', ''))
            obj_widget.strategy_text.setPlainText(obj_data.get('strategy', '')) # ✅ NEW
    ...
    def _on_tab_close_requested(self, index):
        ...
        reply = QMessageBox.question(self, "Delete Objective", f"Are you sure you want to delete '{tab_title}'?\n\nThis cannot be undone.", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            self.tab_widget.removeTab(index)

    def _on_tab_rename_requested(self, index):
        current_name = self.tab_widget.tabText(index)
        new_name, ok = QInputDialog.getText(self, "Rename Objective", "Enter new name:", text=current_name)
    ...
    def _load_existing_data(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Load Error", f"Could not load existing objectives:\n{e}\n\nStarting with a blank objective.")
            self.add_objective()
    ...
    def _on_save(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Saved", f"Successfully saved {len(all_data)} objective(s) to:\n{self.objectives_file}")
            self.accept()
        except Exception as e:
    ...
    def _handle_filter_request(self, observation_widget):
        ...
        filter_dlg = FilterDialog(
            parent=self, incident_path=self.incident_path, data_type=observation_widget.data_type,
            mode="objective", initial_filters=current_filters, plume_data=plume_data
        )
