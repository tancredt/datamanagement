...
# ── Cache stylesheets and styles at module level (call once, reuse everywhere) ──
...
def create_pdf_table_from_df(df):
    ...
    col_widths = [(9.5 * inch) / num_cols] * num_cols
...
def generate_pdf_report(incident_path, parent_widget=None):
    ...
    try:
        ...
        # ✅ Load plume data for the report
        plumes_dir = os.path.join(incident_path, "plumes")
        plume_data = []
        ...
        pdf_path, _ = QFileDialog.getSaveFileName(
            parent_widget, "Save PDF Report", default_filename, "PDF Files (*.pdf);;All Files (*)"
        )
        ...
        def add_observation_header(data_type, summary_str):
            ...
            # Map internal data_type to user-friendly display string
            type_display_map = {
                "area": "Area Data",
                "spot": "Spot Data",
        ...
        for filename, markers in maps_data.items():
            ...
            if os.path.exists(img_path):
                # Render markers onto the map image and save to a temp file for the PDF
                pixmap = QPixmap(img_path)
                rendered_path = save_rendered_map_to_temp(
                    pixmap, markers, 
                    options={"font_size": 12, "circle_radius": 10}, 
        ...
        # Collect unique markers across ALL maps to avoid duplicates
        unique_markers = {}
        for filename, markers in maps_data.items():
            for m in markers:
        ...
        loc_table_data = [
            [
                Paragraph("<b>Label</b>", _STYLE_HEADER_CELL),
                Paragraph("<b>Description</b>", _STYLE_HEADER_CELL),
                Paragraph("<b>Latitude</b>", _STYLE_HEADER_CELL),
                Paragraph("<b>Longitude</b>", _STYLE_HEADER_CELL)
            ]
        ]
        ...
        for zone_name, zone_data in objectives_data.items():
            ...
            for obj in zone_data.get('objectives', []):
                ...
                for obs in observations:
                    ...
                    # ✅ Hide Interval for Spectral, Exposure, Spot, and Plume
                    ...
                    if data_type == 'spectral' and form != 'Table':
                        continue
                    if data_type == 'plume' and form != 'Summary Map':
                        continue
                    ...
                    try:
                        ...
                        if form == 'Table':
                            ...
                            view._render()
                            story.extend(create_pdf_table_from_df(view.filtered_data))
                            ...
                            if data_type == 'plume':
                                # ✅ Plume Rendering Logic
                                start_t = filter_data_dict.get('start_time')
                                stop_t = filter_data_dict.get('stop_time')
                                
                                # Convert string datetimes from JSON back to pandas timestamps
                                if isinstance(start_t, str):
                                    start_t = pd.to_datetime(start_t)
                                if isinstance(stop_t, str):
                                    stop_t = pd.to_datetime(stop_t)
                                    
                                filtered_plumes = []
                                for dt, filepath in plume_data:
                                    if start_t and pd.notna(start_t) and dt < start_t:
                                        continue
                                    if stop_t and pd.notna(stop_t) and dt > stop_t:
                                        continue
                                ...
                                if not filtered_plumes:
                                    ...
                                    # ✅ Iterate with index to insert PageBreaks between images
                                ...
                                # Spot / Area Summary Map
                                view = SummaryMapView(
                                    incident_path=incident_path, data_type=data_type,
                                    map_filenames=list(maps_data.keys()), mapping_dir=mapping_dir, maps_data=maps_data
                                )
                                view.set_filter_summary(filter_data_dict)
                                views._report_stats_pref = filter_data_dict.get("stats_pref", "Mean")
                                view._render()
                                fig = view.render_to_figure()
                                tmp = tempfile.NamedTemporaryFile(suffix='.png', delete=False)
                                tmp_path = tmp.name
                    ...
                    # ── Spacer before PageBreak to prevent content clipping ──
                    story.append(Spacer(1, 0.3*inch))
                    story.append(PageBreak())

        doc.build(
            story,
            onFirstPage=lambda c, d: add_page_number(c, d, "Title Page"),
            onLaterPages=lambda c, d: add_page_number(c, d, footer_label)
        )
