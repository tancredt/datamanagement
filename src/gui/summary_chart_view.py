...
class SummaryChartView(DataView):
    ...
    def _setup_ui(self):
        ...
        # ✅ Add stretch to push the Statistic controls to the right
        metric_layout.addStretch()

        self.lbl_metric = QLabel("<b>Statistic:</b>")
        metric_layout.addWidget(self.lbl_metric)
        ...
        self.summary_figure.set_tight_layout(True)
    ...
    def export(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export Summary Chart", "summary_chart.png",
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(
                    self, "Success",
                    f"Summary chart exported successfully to:\n{file_path}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Error", f"Failed to export summary chart:\n{e}"
                )
    ...
    def _render(self):
        """Calculates summary data using the shared grouping function."""
    ...
    def _redraw(self):
        ...
        if not self.summary_data:
            self.summary_ax.text(
                0.5, 0.5, "No summary data available.",
        ...
        df = pd.DataFrame(
            self.summary_data,
            columns=['Group', 'Analyte', 'Min', 'Max', 'Mean', 'Count', 'DecPls']
        )
        ...
        if pivot_df.dropna(how='all').empty:
            self.summary_ax.text(
                0.5, 0.5, "No plottable data for selected metric.",
                horizontalalignment='center', verticalalignment='center',
                transform=self.summary_ax.transAxes
            )
            self.summary_canvas.draw()
        ...
        if metric_name != 'Count' and active_thresholds:
            for analyte, threshold_val in active_thresholds.items():
                if analyte in pivot_df.columns:
                    self.summary_ax.axhline(
                        y=threshold_val, color='red', linestyle='--',
        ...
        if len(by_label) > 5:
            self.summary_ax.legend(
                by_label.values(), by_label.keys(), fontsize='small', ncol=2
            )
        else:
